...
const FollowBtn = ({ user, color, width, inPost }: IProps) => {
   ...
   const hanleFollow = () => {
      if(auth.user && auth.access_token) {
         ...
         dispatch({ type: AUTH_MODAL, payload: true })
      }
   }
   ...
   const handleAddConversation = () => {
      dispatch(addConversation(user, auth))
            ...
            followed ?
               <div className='d-flex align-items-center'>
                  ...
                  >
                     ...
                     {
                        inPost &&  <span className='ml-1' style={{ fontSize: '16px', fontWeight: '400'}}>Following</span>
                     }
                  </button>
               </div>
               :
               <button 
                  className={`btn btn-${color ? color : 'outline-info'}`}
                  onClick={hanleFollow}
